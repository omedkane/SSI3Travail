...
{
    private readonly Dictionary<Type, dynamic> Tables = new Dictionary<Type, dynamic>();

    private Table<T> TableFromRow<T>(Row<T> row) where T : TableModel => new Table<T>(row);

    private Table<T> TableFromModel<T>(T record) where T : TableModel =>
        TableFromRow<T>(new Row<T>(record));
        ...
        where OldModel : TableModel
    {
        ...
        NewModel newFirstRecord = ObjectHelper.CopyTo<OldModel, NewModel>(
            oldFirstRecord,
            new NewModel(),
            oldModel,
            newModel
        ...
        foreach (Row<OldModel> row in oldRows)
        {
            ...
            NewModel newRecord = ObjectHelper.CopyTo<OldModel, NewModel>(
                row.Record,
                new NewModel(),
                oldModel,
                newModel
