...
public static class ObjectHelper
{
	...
	)
	{
		...
		IEnumerable<string> oldProperties = typeof(TSource).GetProperties().Select(prop => prop.Name);
		IEnumerable<string> newProperties = typeof(TTarget).GetProperties().Select(prop => prop.Name);
	...
	)
	{
		...
		IEnumerable<string> _commonProperties =
			commonProperties ?? GetCommonProperties<TSource, TTarget>(
				typeOfSource,
				typeOfTarget
			);
